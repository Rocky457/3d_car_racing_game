...
def update():
    global offset, run, elapsed_time
    elapsed_time = 0
    if run:
        ...
        if elapsed_time >= 10:
            ...
            offset += time.dt * .3  #rolling background
...
def crash():
    car.animate_rotation_y(720, duration=2)
    Text(text='Crashed! reload the game!',
         origin=(0,0), scale=3, color=color.yellow,background=True)
